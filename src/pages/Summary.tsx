...
export default function SummaryPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      <h1 className="text-2xl font-bold text-foreground mb-6">{t('summary.title')}</h1>

      {/* Duration */}
      <div className="flex gap-2 mb-6">
        {[1, 3, 5, 10].map(d => (
          <button key={d} onClick={() => { setDur(d); setGenerated(d === 5); }}
            className={`h-8 px-4 rounded-lg text-xs font-medium transition-colors ${
              dur === d ? 'bg-foreground text-background' : 'bg-secondary text-muted-foreground hover:text-foreground'
      ...
      {generated ? (
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="rounded-2xl bg-card border border-border p-4">
            ...
            <div className="flex items-center justify-between">
              <span className="font-mono text-[10px] text-muted-foreground">2:06</span>
        ...
        <div className="flex items-center justify-center py-20">
          <button onClick={() => setGenerated(true)}
            className="h-10 px-6 rounded-lg bg-foreground text-background text-sm font-medium hover:opacity-90 transition-opacity">
            {t('summary.generate')}
          </button>
        </div>
      )}
